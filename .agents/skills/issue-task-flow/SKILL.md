...
When project instructions define a delegated Orchestrator workflow, a user request to take the next issue, implement a specific issue, review/control an implementation, complete closure, or close a task is explicit authorization to use that workflow. Do not require the user to repeat the literal words "sub-agent" or "delegate" on every issue. The Orchestrator should remain the human-facing control surface and preserve its context window for planning, handoff quality, review, acceptance, retries/follow-ups, and final accountability.

An Orchestrator must delegate implementation to an Executor sub-agent when sub-agent tools are available and the project is using the delegated flow. The sub-agent is the Executor implementation worker; the Orchestrator should not spend its context window on code/config/docs edits that belong to the Executor. Prefer storing the full task-specific Executor prompt as an `Executor Handoff` Linear comment, then launching the sub-agent with a short bootstrap prompt that points to the issue and handoff comment. If sub-agent tools or Linear handoff access are unavailable, blocked, or inappropriate for the task, fall back to a full separate-chat Executor handoff prompt rather than doing the implementation directly.

An Orchestrator must also delegate mechanical post-acceptance closure to a Closure Executor sub-agent when sub-agent tools are available. The Closure Executor may merge an already accepted task branch, push the integration branch, update tracker status/final notes, and report final repository state, but it must not review, decide acceptance, edit source files, resolve merge conflicts, or broaden scope.
...
The intended human interaction model is: the human talks to the Orchestrator; the Orchestrator starts and controls specialized agents; durable task communication lives in the issue tracker as handoff comments and final notes whenever tracker tooling is available. Direct Orchestrator execution of implementation or mechanical closure is a last-resort fallback only when no sub-agent or separate-chat Executor path is available, or when the user explicitly asks the Orchestrator to do the work personally without agents.
...
When expected tooling is unavailable, times out, lacks dependencies, or cannot see the opened project, prefer a delegated fallback path such as a separate Executor or Closure Executor prompt with the full handoff. Continue with direct Orchestrator implementation or mechanical closure only when no agent path is available or the user explicitly asks for direct Orchestrator work, and report the limitation in the prompt, review, or final result.
