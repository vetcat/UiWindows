...
{
    internal sealed class UiRuntimeWindowSource<TWindow, TView> : IDisposable
        ...
        where TView : WindowComponent
    {
        ...
        private UiRuntimeWindowSource(TWindow windowSource, WindowLayout layoutSource)
        {
            WindowSource = windowSource;
            this.layoutSource = layoutSource;
        }

        public TWindow WindowSource { get; }

        public static UiRuntimeWindowSource<TWindow, TView> Create(
            TView viewPrefab,
            string windowName,
            int layoutTagId,
            bool takeFocus)
        {
            ...
            var layout = CreateLayoutSource(windowName, layoutTagId);
            var window = CreateWindowSource(layout, viewPrefab, windowName, layoutTagId, takeFocus);
            return new UiRuntimeWindowSource<TWindow, TView>(window, layout);
        }
        ...
        private static TWindow CreateWindowSource(
            WindowLayout layout,
            TView viewPrefab,
            string windowName,
            int layoutTagId,
            bool takeFocus)
        {
            var gameObject = new GameObject(windowName + " Source", typeof(RectTransform));
            UnityEngine.Object.DontDestroyOnLoad(gameObject);

            var window = gameObject.AddComponent<TWindow>();
            ...
            window.preferences.takeFocus = takeFocus;
            ...
            var cameraObject = new GameObject(windowName + " Camera", typeof(Camera));
        ...
        private static WindowLayout CreateLayoutSource(string windowName, int layoutTagId)
        {
            var gameObject = new GameObject(
                windowName + " Layout Source",
            ...
            var elementObject =
                new GameObject(windowName + " Slot", typeof(RectTransform), typeof(WindowLayoutElement));
        ...
        private static Resource CreateDirectComponentResource(TView viewPrefab)
        {
            return new Resource
            {
